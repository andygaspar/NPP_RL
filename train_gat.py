...
for iteration in range(ITERATIONS):
    P, C = random.choice(N_PATHS), random.choice(N_COMM)
    instances = [GATInstance(P, C, seed=0) for i in range(EPISODE_PER_BATCH)]
    ...
    for i, inst in enumerate(instances):
        ...
        log_prices.append(log_probs[:, mask])

        g = Genetic(inst, pop_size=POPULATION, verbose=False)
        g.run(GA_ITERATIONS)
        baselines.append([g.best_val for _ in range(inst.n_paths * N_SAMPLES)])

        g_nn = Genetic(inst, pop_size=POPULATION, verbose=False)
        g_nn.run(1, init_population = inst.get_prices_from_net(samples_graph))
        # rewards.append([g_nn.best_val for _ in range(inst.n_paths * N_SAMPLES)])
        rewards.append(np.repeat(g_nn.final_vals, inst.n_paths).tolist())
        wins += g.best_val < g_nn.best_val

    reward_tensor = torch.tensor(rewards, dtype=torch.float32).flatten()
    baseline_tensor = torch.tensor(baselines, dtype=torch.float32).flatten()
    log_prices = torch.cat(log_prices).flatten()
