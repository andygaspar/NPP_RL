...
class SKWP:
    ...
    def solve_all(self, verbose=False, time_limit=None):
        ...
        w_k = self.model.addMVar((self.inst.K, self.inst.F + 2, self.inst.L), name='w')
        x = self.model.addMVar((self.inst.K, self.inst.F + 2, self.inst.L), vtype=GRB.BINARY, name='x')
        y = self.model.addMVar((self.inst.K, self.inst.F + 2), vtype=GRB.BINARY, name='y')
        p_F = np.zeros((self.inst.K, self.inst.F + 2))
        w_F = np.zeros((self.inst.K, self.inst.F + 2))
        for k in range(self.inst.K):
            idx_eff = np.argsort(-self.inst.p[k, self.inst.L:] / self.inst.w[k, self.inst.L:])

            p_F[k, 0] = self.inst.p[k].max() + 1000
            p_F[k, -1] = 1  # self.inst.p.min() / 2
            sorted_p = self.inst.p[k, idx_eff + self.inst.L]
            p_F[k, 1: -1] = sorted_p

            w_F[k, 0] = self.inst.w[k].min() / 2
            w_F[k, -1] = self.inst.c[k]
            sorted_w = self.inst.w[k, idx_eff + self.inst.L]
            w_F[k, 1: -1] = sorted_w

            cap_adjusted = self.c[k] + w_F[k, 0]

            self.model.addConstr(x[k].sum(axis=0) <= 1, name='x < 1 ' + str(k) + ' ')
            for i in range(self.inst.F + 2):
                for j in range(self.inst.L):
                    self.model.addConstr(x[k, i, j] + y[k, i] <= 1, name='x + y < 1 ' + str(k) + ' ' + str(i) + ' ' + str(j))

                    self.model.addConstr(w_k[k, i, j] <= (self.inst.p[k, j] * w_F[k, i] / p_F[k, i]) * x[k, i, j],
                                         name='p * w / p ' + str(k) + ' ' + str(i) + ' ' + str(j))

            self.model.addConstr(
                cap_adjusted * (1 - y[k, 0]) - w_k[k, 0, :].sum() <= w_F[k, 0] - 1e-7, name='< w_F 0' + str(k) + ' ')

            for i in range(1, self.inst.F + 2):
                self.model.addConstr(
                    cap_adjusted * (1 - y[k, i]) - w_k[k, :i + 1, :].sum() - (w_F[k, :i] * y[k, :i]).sum() <=
                    w_F[k, i] - 1e-7, name='< w_F --' + str(k) + ' ' + str(i))

            self.model.addConstr(w_k[k].sum() + (w_F[k] * y[k]).sum() <= cap_adjusted, name='cap ' + str(k) + ' ')

            self.model.addConstr(w == w_k[k].sum(axis=0), name='w <= w_k' + str(k) + ' ')
